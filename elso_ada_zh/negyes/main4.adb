...
procedure Main4 is
	...
	function Testing_Four_Marks return Boolean is
		...
		begin
			...
			for I in Integer range 1..4 loop
				Integer_Maps.Insert(Int_Maps_7,I,I+2);
			end loop;
			if Integer_Maps.Size(Int_Maps_7) /= 4 then
				return False;
			end if;
			...
			if Integer_Maps.Size(Int_Maps_7) /= 6 then
				return False;
			end if;
			if exc_count /= 2 then
				return False;
	...
	begin
		return
			...
			Correct(Search_Odd'Access,Int_Array_Even_Cond_True, False, 1) and then
			Map_Test;
	end Testing_Four_Marks;

begin
	if not Testing_Four_Marks then
